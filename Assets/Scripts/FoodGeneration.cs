...
public class FoodGeneration : MonoBehaviour
{
    ...
    public float XSize = 12f; //Размер поля по X
    public float ZSize = 6f; //Размер поля по Z
...
//		float ZSize = 6f;
    ...
    void Update()
    {
        ...
        { //Максимальное количество еды после 5 собранных = 2

            maxFoodInField = 2;

            if (currentFoodInField < maxFoodInField)
            { //Спауним пока не превысит максимальное количество
        ...
        if (FoodCount >= 15)
        { //Более 15 съеденной еды - спауним по 3
